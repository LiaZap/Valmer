...
export function FormDegustacao({
  ...
  function enviar(evento: React.FormEvent) {
    ...
    iniciarEnvio(async () => {
      ...
      setEmail('')
      router.refresh()
      toast(`Degustação criada para ${avaliado}. O link está em Mapas comportamentais.`)
    })
  }
      ...
      <Card padding="lg">
        <form onSubmit={enviar} className={styles.config}>
          ...
          {/* Região viva permanente: o aviso e a recusa chegam depois do clique,
              ...
              assessments/novo/FormNovoAssessment.tsx. */}
          <div role="status" aria-live="polite">
            {semSaldo ? (
              <div className={`${ui.callout} ${ui.calloutWarning}`}>
                ...
                <span>
                  Sem degustações disponíveis: restam {saldo} de {concedidas}. Peça mais amostras
                  ao administrador, ou envie este mapa pela tela de mapas comportamentais, com
                  crédito.
                </span>
              </div>
            ) : null}
          ...
          >
            {enviando ? 'Enviando…' : 'Enviar degustação'}
          </Button>
        </form>
      </Card>
    </AutoGrid>
