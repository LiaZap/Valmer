...
export function ListaDevolutivas({
  ...
  function abrir() {
    ...
    iniciarGravacao(async () => {
      ...
      toast('Devolutiva aberta. O tempo começa a contar agora.')
    })
  }
  ...
  function finalizar(item: ItemDevolutiva) {
    iniciarGravacao(async () => {
      ...
      toast(`Devolutiva de ${item.nome} finalizada.`)
    })
  }
  ...
  return (
    <>
      ...
      <Card>
        ...
        {concluidos.length === 0 ? (
          <p className={ui.note}>
            Nenhum mapa concluído sem devolutiva. A sessão de feedback abre a partir de um mapa
      ...
      <Card padding="none" scrollX>
        ...
        {itens.length === 0 ? (
          <EmptyState>
            <p>
              Você ainda não tem devolutivas. Escolha um mapa concluído acima para abrir a
              primeira sessão de feedback.
            </p>
          </EmptyState>
        ) : (
          <Table>
