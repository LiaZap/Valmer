...
type LiderancaProps = {
  ...
  /**
   * As três seções deste arquivo entram em níveis DIFERENTES: encaixe é
   * S1, e as duas de liderança são S2. Por isso o corte é por seção e não
   * pelo componente inteiro. Gatear tudo junto tirava "Onde você se
   * encaixa" do S1, que a tabela de preços vende como parte do S1.
   */
  mostrarEncaixe: boolean
  mostrarLideranca: boolean
            ...
            <div className={styles.painel}>
              ...
              <div className={styles.painelCorpo}>
                <p className={styles.destinatario}>
                  ...
                  <span>
                    Daqui em diante o texto muda de interlocutor. Ele fala com quem conduz o
                    trabalho de {nome}, sobre {nome}. Esta página existe para ser mostrada e
                    conversada abertamente.
                  </span>
                </p>
                ...
                <ul className={styles.orientacoes}>
                  {perfil.oQueEvitar.map((evitar) => (
                    <li key={evitar} className={[styles.orientacao, styles.evitar].join(' ')}>
                      <span
                        className={[styles.orientacaoIcone, styles.evitarIcone].join(' ')}
                        aria-hidden
                      >
